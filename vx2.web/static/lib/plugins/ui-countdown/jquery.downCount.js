...
(function($) {
	$.fn.downCount = function(options, callback) {
		...
		var currentDate = function() {
			...
			//var date = new Date();//Fri Jun 5 2016 15:45:26 GMT+0800 (中国标准时间)

			//get server's current date
			var utc =parseInt(settings.nowDate)+parseInt(count*1000);
			// turn date to utc
			//var utc = date.getTime() + (date.getTimezoneOffset() * 60000);//时间戳
			// set new Date object
		...
		function countdown() {
			...
			//按小时计算
			//var days = Math.floor(difference / _day), hours = Math.floor(difference/ _hour), minutes = Math.floor((difference % _hour) / _minute), seconds = Math.floor((difference % _minute) / _second);
